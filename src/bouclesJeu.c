...
void boucle_jeu_espace(sommet_t **tab, int n, int *chemin, int n_chemin, int* close)
{
    ...
    while (program_on)
    {
        ...
        while (!isInPath_line(x, y, sous_graphe, n, PATH_SIZE-10, 0) && !isInPath_line(x-32, y-32, sous_graphe, n, PATH_SIZE-10, 0))
        {
            x -= tmpSpeedX*2;
            y -= tmpSpeedY*2;
            speedX = 0;
...
int mur_proche(Point p, sommet_t ** tab, int n, int depth, int precision){
    ...
    for (int i = 0; i < 4; i++){
        ...
        {
            if (isInPath_line(p.x + direction[i].x*j, p.y + direction[i].y*j, tab, n, PATH_SIZE-10, 0)){
                bestDist = j;
                closest = i;
            }
        }
...
int is_mur_in_between(Point p1, Point p2, sommet_t ** tab, int n, int precision){
    ...
    {
        if (isInPath_line(p1.x + direction[dir_noeud].x*i, p1.y + direction[dir_noeud].y*i, tab, n, PATH_SIZE-10, 0)){
            return 1;
        }
    }
    return 0;
